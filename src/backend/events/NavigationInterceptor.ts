...
import { openFolderNote, createFolderNote } from '../core/FolderNoteService';
...
export async function handleFolderNoteReveal(plugin: FolderNotesPlugin, folderNote: TFile): Promise<void> {
	const fileExplorerPlugin = (plugin.app as any).internalPlugins?.getEnabledPluginById('file-explorer');
...
export async function handleFolderNoteCreation(
	event: MouseEvent,
	plugin: FolderNotesPlugin,
	folderPath: string,
): Promise<boolean> {
