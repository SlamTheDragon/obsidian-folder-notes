...
import { addExcludedFolder } from '../../backend/core/ExcludeService';
import { ExcludeFolderModal } from '../modals/ExcludeFolderModal';
import { PatternModal } from '../modals/PatternModal';
import { WhitelistedFoldersModal } from '../modals/WhitelistedFoldersModal';
import {
	addExcludeFolderListItem,
	addExcludePatternListItem,
} from './ExcludedFolderListItems';
...
export async function renderExcludeFolders(settingsTab: SettingsTab): Promise<void> {
	...
	new Setting(containerEl)
		...
		.addButton((cb) => {
			...
			cb.setCta();
			cb.onClick(async () => {
				new WhitelistedFoldersModal(settingsTab).open();
			});
		});
	...
	settingsTab.plugin.settings.excludeFolders
		...
		.forEach((excludedFolder) => {
			if (excludedFolder.string?.trim() !== '' && excludedFolder.path?.trim() === '') {
				addExcludePatternListItem(settingsTab, containerEl, excludedFolder as any);
			} else {
				addExcludeFolderListItem(settingsTab, containerEl, excludedFolder as any);
			}
		});
}

export default renderExcludeFolders;
